...
A single AST containing all language constructs: operational types (`Group`, `Ring`, `Field`), simplicial type theory variables, and directed type theory linear tensors/binders.
...
A clean surface language that allows defining operational structures as high-level declarations. The compiler then elaborates/compiles these declarations into corresponding expressions in `Ulrik` and `Mike`'s ASTs.
...
Operational structures are defined as terms of predefined algebraic record types in `Ulrik`/`Mike` (e.g. `Record Group := { G : Type; mult : G -> G -> G; ... }`).
