...
## 7. Decision: GAP-Compatible Type Extensions (Reviewer 1)

### 7.1 Proposal Overview
Reviewer 1 proposed a revised and significantly strengthened version of the term and hypothesis types for the operational `Dan` language to support concrete computational group and representation theory constructs similar to actual GAP (Groups, Algorithms, and Programming) data structures.

### 7.2 Proposed AST Structure
The proposed AST introduces first-class representations for:
1. **Permutations & Cycles**:
   - `Perm of int list` and `Cycle of int list list` to represent elements in symmetric groups directly.
   - `PermGroup of term list` for groups generated by a collection of permutations.
2. **Matrix Groups**:
   - `MatGroup of term list * int * int` to represent groups defined by generator matrices over finite fields of size $q$ and dimension $n$ (such as $GL_n(\mathbb{F}_q)$).
3. **Advanced Presentation Types**:
   - `FpGroup` for finitely presented groups $\langle X \mid R \rangle$.
   - `PcGroup` for polycyclic presentations, facilitating soluble group computations.
4. **Group Action Dynamics**:
   - `Conj` (conjugation $g^h = h^{-1} g h$) and `Comm` (commutator $[g, h]$).
   - Group actions (`Act`, `Action`), along with `Orbit` and `Stabilizer` computation assertions.
5. **Topological Map Primitives**:
   - First-class terms for `Face`, `Deg`, and `Boundary` operators.

### 7.3 Architectural Assessment & Decision
While this enhanced AST provides closer semantic alignment with GAP and allows expressing advanced algebra theorems (e.g. the Stabilizer-Orbit Theorem), we decided **not to fully incorporate it immediately in the active prototype codebase**.

**Rationale:**
1. **Elaboration Complexity**: Translating permutation elements, polycyclic relations, and matrix groups over finite fields into **Riehl-Shulman Simplicial Type Theory (Ulrik)** or **Directed Type Theory (Mike)** is a massive compilation task. It would require building a complete representation and field theory library in the backend type systems.
2. **Typechecker Bloat**: It would shift `dan.ml` from a simple, linear-time presentation checker into a full-scale computer algebra engine, violating the minimal kernel principle.

### 7.4 Phased Plan
1. **Documentation (Current)**: Keep the design preserved in the project history.
2. **Selective Implementation**: In future development phases, selectively implement group operators like conjugation (`Conj`) and commutators (`Comm`) as they can be elaborated directly into composition and path induction terms without requiring full permutation or finite field algebra backends.
